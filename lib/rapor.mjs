...
export function buildRapor({ from, to, events, filters = {}, generatedBy, logo, now = new Date() }) {
  ...
  const scope = [filters.city || 'Tüm kapsamlar', filters.theme || 'Tüm çalışma grupları', (filters.subtype || 'Tüm etkinlik türleri'), filters.status || 'Tüm durumlar', filters.search && `“${filters.search}” araması`].filter(Boolean).join('  ·  ');
  ...
  if (events.length) {
    ...
    body += para(run(`${title} için ${filters.city || filters.theme || filters.category || filters.status || filters.search ? 'seçilen süzgeçlerle eşleşen ' : ''}kayıtlı etkinlik bulunmuyor.`, { color: MUTED }));
  }
