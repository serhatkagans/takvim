...
import { categories, subtypes, groups, listOf, OTHER_KIND, RENAMED_GROUPS, RENAMED_SUBTYPES } from './data.mjs';
...
/**
 * Kayıtlardaki tür, alt tür ve çalışma gruplarını güncel listelere uydurur:
...
async function normalizeSubtypes(db) {
  ...
  for (const e of await db.all('SELECT id,category,subtypes,work_groups FROM events')) {
    const current = listOf(e.subtypes), currentGroups = listOf(e.work_groups), own = subtypes[e.category] || [];
    ...
    const valid = categories.includes(e.category) ? e.category : OTHER_KIND;
