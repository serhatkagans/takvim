...
import { categories, subtypes, listOf, GROUP_KIND, RENAMED_SUBTYPES } from './data.mjs';
...
/**
 * Kayıtlardaki alt türleri güncel listelere uydurur: eski adlar yeni
 * karşılığına çevrilir (gerekirse etkinlik o adın türüne taşınır), karşılığı
 * olmayanlar kaldırılır. Yalnızca listede olmayan değer taşıyan satırlara
 * dokunur; listeler değişmedikçe her açılışta hiçbir şey yapmaz.
 */
async function normalizeSubtypes(db) {
  ...
  for (const e of await db.all('SELECT id,category,subtypes FROM events')) {
    const current = listOf(e.subtypes), own = subtypes[e.category] || [];
    if (current.every(name => own.includes(name))) continue;
    const found = current.map(name => (own.includes(name) ? [e.category, name] : locate(name))).filter(Boolean);
    const kind = !found.length || found.some(([k]) => k === e.category) ? e.category : found[0][0];
    const names = found.filter(([k]) => k === kind).map(([, name]) => name);
    const ordered = (subtypes[kind] || []).filter(name => names.includes(name));
    await db.run('UPDATE events SET category=?,subtypes=?,theme=? WHERE id=?',
      [kind, `|${ordered.join('|')}|`, kind === GROUP_KIND && ordered.length ? ordered.join(', ') : 'Genel', e.id]);
  }
}
