 ...
 *  bölgesel / yerel etkinlik bir veya daha çok ile bağlanır. Eski "İl" ve
 *  "Ortak iller" kayıtları açılışta bölgesel / yerel olur (bkz. lib/db.mjs).
...
/** Ana türlerin dışında kalan etkinlikler. Alt türü yoktur. */
export const OTHER_KIND = 'Diğer';
/* v2.8: "İl etkinliği" türü kaldırıldı; eski kayıtları açılışta "Diğer" olur. */
export const categories = ['Temel GençTek etkinliği', 'Görünürlük / Tanıtım', OTHER_KIND];
...
/* Görünürlük / tanıtım türünün alt seçeneği yok. Liste boşken bu türde alt
   seçenek sorulmaz; liste eklendiğinde seçim kendiliğinden zorunlu olur. */
const PROMOTION_TYPES = [];
export const BASIC_KIND = 'Temel GençTek etkinliği';
export const subtypes = { [BASIC_KIND]: BASIC_TYPES, 'Görünürlük / Tanıtım': PROMOTION_TYPES, [OTHER_KIND]: [] };
...
export const validFilters = f => (!f.city || places.includes(f.city)) && (!f.theme || groups.includes(f.theme)) && (!f.category || categories.includes(f.category))
  ...
  && (!f.subtype || (f.category && subtypes[f.category].includes(f.subtype)));
 ...
 *
 * YETKİ: il yöneticisi bölgesel / yerel etkinliği yalnızca kendi ilini
 * içeriyorsa kaydeder. Ulusal ve uluslararası etkinlik bir ile bağlı olmadığı
 * için onu da açabilir; sonradan yalnızca kendi açtığını düzenler.
 */
export function validateEvent(body, user) {
  ...
  if ((options.length && !picked.length) || picked.some(s => !options.includes(s))) gecersiz('İlişkili olduğu en az bir etkinlik türünü seçin.');
  ...
  if (e.scope === REGIONAL) {
    ...
    if (selected.some(c => !eventCities.includes(c)) || !selected.length) gecersiz('Bölgesel / yerel etkinlik için en az bir il seçin.');
    /* YEĞİTEK il değil: sıralamada her zaman başta durur. */
    chosenCities = [...new Set(selected)].sort((a, b) => (a === CENTER ? -1 : b === CENTER ? 1 : a.localeCompare(b, 'tr')));
  }
  e.cities = chosenCities.length ? `|${chosenCities.join('|')}|` : '';
  e.city = chosenCities.length ? chosenCities.join(', ') : e.scope;
  /* İl yöneticisi il bağlı bir etkinliği yalnızca kendi ilini içeriyorsa
     kaydeder. Ulusal ve uluslararası etkinlik hiçbir ile bağlı değildir:
     onu da açabilir, sonra yalnızca kendi açtığını düzenleyebilir. */
  if (user.city && chosenCities.length && !chosenCities.includes(user.city))
    gecersiz(`Yalnızca ${user.city} ilini içeren etkinlik kaydedebilirsiniz.`);
