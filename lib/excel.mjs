...
const COLUMNS = [
  ...
  ['Düzenleyen iller', 24, 'text', e => listOf(e.cities).join(', ')],
  ['Katılan iller', 24, 'text', e => listOf(e.participants).join(', ')],
...
/** Sütun harfi: 0 → A, 25 → Z, 26 → AA (form yanıtlarında soru sayısı 26'yı aşabilir). */
const column = index => (index >= 26 ? column(Math.floor(index / 26) - 1) : '') + String.fromCharCode(65 + (index % 26));

function cell(ref, kind, value) {
  if (value === '' || value === null || value === undefined) return '';
  if (kind === 'text') return `<c r="${ref}" s="${STYLE.text}" t="inlineStr"><is><t xml:space="preserve">${esc(value)}</t></is></c>`;
 ...
 * @returns {Buffer} .xlsx
 */
export const buildExcel = ({ events, now = new Date() }) => buildSheet({ name: SHEET, columns: COLUMNS, rows: events, now });

/**
 * Tek sayfalık çalışma kitabı: başlık satırı sabit ve otomatik süzgeçli.
 * `columns` öğeleri [başlık, genişlik, tür (text | date | number), değer(satır)].
 * Boş değer hücre yazılmadan geçilir. Sayfa adı en fazla 31 karakter, []:*?/\ içermez.
 */
export function buildSheet({ name, columns, rows, now = new Date() }) {
  const range = `$A$1:$${column(columns.length - 1)}$${rows.length + 1}`;
  const header = `<row r="1">${columns.map(([label], i) => `<c r="${column(i)}1" s="${STYLE.header}" t="inlineStr"><is><t>${esc(label)}</t></is></c>`).join('')}</row>`;
  const body = rows.map((row, index) => `<row r="${index + 2}">${columns.map(([, , kind, pick], i) => cell(`${column(i)}${index + 2}`, kind, pick(row))).join('')}</row>`).join('');
  const sheet = `${XML}<worksheet xmlns="${MAIN}" xmlns:r="${REL}">`
    ...
    + `<sheetData>${header}${body}</sheetData><autoFilter ref="${range.replace(/\$/g, '')}"/></worksheet>`;
  const workbook = `${XML}<workbook xmlns="${MAIN}" xmlns:r="${REL}"><sheets><sheet name="${esc(name)}" sheetId="1" r:id="rId1"/></sheets>`
    + `<definedNames><definedName name="_xlnm._FilterDatabase" localSheetId="0" hidden="1">'${esc(name)}'!${range}</definedName></definedNames></workbook>`;
