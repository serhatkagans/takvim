...
export function buildIcs(events, { name = 'GençTek Etkinlik Takvimi', host = 'genctek' } = {}) {
  ...
  for (const e of events) {
    ...
    const description = [e.purpose && `Amaç: ${e.purpose}`, e.description, `Etkinlik türü: ${listOf(e.subtypes).join(', ')}`, listOf(e.work_groups).length && `Çalışma grubu: ${listOf(e.work_groups).join(', ')}`, partners && `Paydaş / İşbirliği Yapılan Kurum: ${partners}`, e.status !== 'Planlandı' && `Durum: ${e.status}`]
      .filter(Boolean).join('\n\n');
